...
server <- function(input, output, session) {
  ...
  # update infomation boxes (top view)
  output$infoboxes <- renderUI({
    if(isTruthy(rv$filtered_data)){
      gen_summary_boxes(rv$filtered_data)
    } 
  })
  
  
  # update filtered data frame
  observeEvent(c(input$publication_types,
  ...
  observeEvent(rv$data, {
    rv$publication_summary <- rv$data %>%
      ...
      summarise(n_pubs = n(), n_citations = sum(cited_count), .groups = "drop") %>%
  ...
  output$trend_chart<- renderHighchart({
    if(isTruthy(rv$filtered_data)){
      plot_publication_trend(rv$filtered_data, input$gs_chk_total_citation)
    }
  })
