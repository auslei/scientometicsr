...
author_communities <- as.data.frame(list(author=names(V(sub_g)), community=sub_wc$membership), 
                                    stringsAsFactors=FALSE)

df_merged <- df %>% merge(author_communities, by = "author") %>% 
                    select(doc_id, author, community, title, abstract, year, raw_text, clean_text, formated_keywords)

plot_wordcloud(filter(df_merged, community == 1), type = "clean")
...
for(i in c(1, 2, 12, 6)) {
  print(paste("## Top score for community ", i))
...
silos %>% select(author, title) %>% kable()
```